...
class StrategyLearner(object):  		  	   		 	 	 			  		 			     			  	 
    ...
    def add_evidence(  		  	   		 	 	 			  		 			     			  	 
        ...
        # returns_df['Action'] = np.where(returns_df['N_day_ret'] > 0.02, 1, np.where(returns_df['N_day_ret'] < -.02, -1, 0))
        Y = df['N_day_ret'].to_numpy()
        # print(Y)
    ...
    def testPolicy(  		  	   		 	 	 			  		 			     			  	 
        ...
        # preds_df['Shares'] = np.where(preds_df['Action'] > 0.4, 1,
        #                             np.where(preds_df['Action'] < -0.4, -1, 0))
